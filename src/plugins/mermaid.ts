...
export const remarkMermaid: RemarkPlugin<[]> = () => tree => {
  visit(tree, "code", node => {
    ...
    // @ts-ignore
    node.type = "html";
    node.value = `<div class="mermaid flex justify-center bg-background" data-status="loading"><p class="prompt">Loading mermaid graph...</p><pre class="src" data-mermaid-src>${escapeSrc}</pre></div>`;
  });
};
